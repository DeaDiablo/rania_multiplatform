...
public class SideBar extends Group implements Part {
	...
	public void loadPart() {
		...
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHAT_OFF, 192, 192, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png", RegionID.BTN_UI_CHAT_ON,
				192, 288, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_MENU_OFF, 288, 192, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png", RegionID.BTN_UI_MENU_ON,
				288, 288, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHANNEL_MAIN_OFF, 0, 0, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHANNEL_MAIN_ON, 0, 96, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHANNEL_LOCATION_OFF, 96, 0, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHANNEL_LOCATION_ON, 96, 96, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHANNEL_PLANET_OFF, 192, 0, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHANNEL_PLANET_ON, 192, 96, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHANNEL_DOMAIN_OFF, 288, 0, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHANNEL_DOMAIN_ON, 288, 96, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHANNEL_PRIVATE_OFF, 0, 192, 96, 96, false);
		mView.loadTexture("data/gui/icon_collect.png",
				RegionID.BTN_UI_CHANNEL_PRIVATE_ON, 0, 288, 96, 96, false);
		mView.loadTexture("data/gui/ui_back.png", RegionID.BTN_UI_BACK_OFF, 0,
				0, 450, 100, false);
		mView.loadTexture("data/gui/ui_back.png", RegionID.BTN_UI_BACK_ON, 0,
				100, 450, 100, false);
		mView.loadTexture("data/gui/ui_exit.png", RegionID.BTN_UI_EXIT_OFF, 0,
				0, 450, 100, false);
		mView.loadTexture("data/gui/ui_exit.png", RegionID.BTN_UI_EXIT_ON, 0,
				100, 450, 100, false);
		...
		mView.loadTexture("data/gui/btn_send.png", RegionID.BTN_UI_SEND_OFF, 0,
				0, 128, 64, false);
		mView.loadTexture("data/gui/btn_send.png", RegionID.BTN_UI_SEND_ON, 0,
				64, 128, 64, false);

		float halfWidth = mView.getHUDCamera().getWidth() * 0.5f;
		float halfHeight = mView.getHUDCamera().getHeight() * 0.5f;

		panelBlank = new PressedButton(RegionID.BLANK, RegionID.BLANK, 0.0f,
				0.0f);

		btnDisconnect = new PressedButton(RegionID.BTN_UI_EXIT_OFF,
				RegionID.BTN_UI_EXIT_ON, halfWidth * 0.0f,
				-halfHeight * 0.137f, new TouchAction() {
					@Override
		...
		btnCancel = new PressedButton(RegionID.BTN_UI_BACK_OFF,
				RegionID.BTN_UI_BACK_ON, halfWidth * 0.0f, halfHeight * 0.137f,
				new TouchAction() {
					@Override
					public void execute(boolean touch) {
						setVisible(panel);
						...
						RaniaGame.mController.addProcessor(lController
								.getPlayerController());
					}
				});

		btnMenu = new PressedButton(RegionID.BTN_UI_MENU_OFF,
				RegionID.BTN_UI_MENU_ON, halfWidth * 0.9396f,
				-halfHeight * 0.8926f, new TouchAction() {
					@Override
					public void execute(boolean touch) {
						setVisible(menu);
						lController.getRadar().visible = false;
						Parts.getInfoPanel().setVisible(false);
						RaniaGame.mController.removeProcessor(lController
								.getPlayerController());
					}
				});

		btnChat = new PressedButton(RegionID.BTN_UI_CHAT_OFF,
				RegionID.BTN_UI_CHAT_ON, halfWidth * 0.9396f,
				-halfHeight * 0.6963f, new TouchAction() {
					@Override
					public void execute(boolean touch) {
		...
		fieldChat = new ChatList(
				RegionID.FIELD_CHAT,
				-halfWidth
						+ mView.getTextureRegion(RegionID.FIELD_CHAT)
								.getRegionWidth() * 0.5f,
				halfHeight
						- mView.getTextureRegion(RegionID.FIELD_CHAT)
								.getRegionHeight() * 0.5f,
				new MultilineText("", Font.getFont("data/fonts/Arial.ttf", 20),
						new Color(1, 1, 1, 1),
						-mView.getTextureRegion(RegionID.FIELD_CHAT)
								.getRegionWidth() * 0.5f + 20.0f, mView
								.getTextureRegion(RegionID.FIELD_CHAT)
								.getRegionHeight() * 0.5f - 10.0f,
		...
		editUser = new Edit(RegionID.EDIT_CHAT, RegionID.EDIT_CHAT, -halfWidth
				+ mView.getTextureRegion(RegionID.EDIT_CHAT).getRegionWidth()
				* 0.5f, -halfHeight
				+ mView.getTextureRegion(RegionID.EDIT_CHAT).getRegionHeight()
				* 0.5f, new Text("", Font.getFont("data/fonts/Arial.ttf", 20),
				new Color(1, 1, 1, 1), -mView.getTextureRegion(
						RegionID.EDIT_CHAT).getRegionWidth() * 0.5f + 20.0f, 0,
				Text.Align.LEFT, Text.Align.CENTER), 255, new EditAction() {
			@Override
			public void execute(Edit edit) {
				Controllers.netController.sendChatMessage(edit.getText(),
						fieldChat.getCurrentChannel());
				edit.setText("");
				edit.setFocus();
			}
		});
		...
		btnSend = new PressedButton(RegionID.BTN_UI_SEND_OFF,
				RegionID.BTN_UI_SEND_ON, -halfWidth
						+ mView.getTextureRegion(RegionID.EDIT_CHAT)
								.getRegionWidth()
						+ mView.getTextureRegion(RegionID.BTN_UI_SEND_OFF)
								.getRegionWidth() * 0.5f, editUser.position.y,
				new TouchAction() {
					@Override
					public void execute(boolean touch) {
		...
		btnMain = new RadioButton(RegionID.BTN_UI_CHANNEL_MAIN_OFF,
				RegionID.BTN_UI_CHANNEL_MAIN_ON, halfWidth * 0.9396f,
				halfHeight
						- mView.getTextureRegion(
								RegionID.BTN_UI_CHANNEL_MAIN_OFF)
								.getRegionHeight() * 0.75f, new TouchAction() {
					@Override
					public void execute(boolean touch) {
		...
		btnLocation = new RadioButton(RegionID.BTN_UI_CHANNEL_LOCATION_OFF,
				RegionID.BTN_UI_CHANNEL_LOCATION_ON, halfWidth * 0.9396f,
				halfHeight
						- mView.getTextureRegion(
								RegionID.BTN_UI_CHANNEL_MAIN_OFF)
								.getRegionHeight() * 2.0f, new TouchAction() {
					@Override
					public void execute(boolean touch) {
		...
		btnPlanet = new RadioButton(RegionID.BTN_UI_CHANNEL_PLANET_OFF,
				RegionID.BTN_UI_CHANNEL_PLANET_ON, halfWidth * 0.9396f,
				halfHeight
						- mView.getTextureRegion(
								RegionID.BTN_UI_CHANNEL_MAIN_OFF)
								.getRegionHeight() * 3.25f, new TouchAction() {
					@Override
					public void execute(boolean touch) {
		...
		btnDomain = new RadioButton(RegionID.BTN_UI_CHANNEL_DOMAIN_OFF,
				RegionID.BTN_UI_CHANNEL_DOMAIN_ON, halfWidth * 0.9396f,
				halfHeight
						- mView.getTextureRegion(
								RegionID.BTN_UI_CHANNEL_MAIN_OFF)
								.getRegionHeight() * 4.5f, new TouchAction() {
					@Override
					public void execute(boolean touch) {
		...
		btnPrivate = new RadioButton(RegionID.BTN_UI_CHANNEL_PRIVATE_OFF,
				RegionID.BTN_UI_CHANNEL_PRIVATE_ON, halfWidth * 0.9396f,
				halfHeight
						- mView.getTextureRegion(
								RegionID.BTN_UI_CHANNEL_MAIN_OFF)
								.getRegionHeight() * 5.75f, new TouchAction() {
					@Override
					public void execute(boolean touch) {
