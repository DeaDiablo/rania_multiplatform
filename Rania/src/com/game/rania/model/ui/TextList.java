...
public class TextList extends HUDObject {
	...
	public TextList(RegionID idTexture, float x, float y, MultilineText text,
			float widthList, float heightList) {
		super(idTexture, x, y);
		this.text = text;
		...
		if (text != null) {
			for (int i = beginLine; i < endLine; i++) {
				...
				if (line != null) {
					...
					text.draw(sprite, text.position.x + position.x,
							text.position.y + position.y - (i - beginLine)
									* text.font.getLineHeight(), angle,
							scale.x, scale.y);
				}
			}
		}
