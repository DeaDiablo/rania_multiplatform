...
public class Planet extends Object {
	...
	public Planet(int id, String name, int type, int radius, int speed,
			int orbit, int idLocation, int Domain, int ASX, int ASY) {
		...
		updatePosition();
		shader = Controllers.shaderManager.getShader("data/shaders/main.vert",
	...
	public void updatePosition() {
		...
		calcPosition(time);
		if (region != null)
			scale.set(2.0f * radius / region.getRegionWidth(), 2.0f * radius
	...
	public boolean draw(SpriteBatch sprite) {
		...
		if (currentShader == shader && shader != null) {
			shader.setUniformf("uvMin",
					new Vector2(region.getU(), region.getV()));
			shader.setUniformf("uvMax",
					new Vector2(region.getU2(), region.getV2()));
	...
	private void calcPosition(float currentTime) {
		...
		position.set((float) Math.cos(calcAngle), (float) Math.sin(calcAngle));
		position.mul(orbit);
