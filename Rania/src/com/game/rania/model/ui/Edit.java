...
public class Edit extends HUDObject {
	...
	public Edit(RegionID idTexture, RegionID idTextureOn, float x, float y,
			Text text, int maxTextVisible) {
		super(idTexture, x, y);
	...
	public Edit(RegionID idTexture, RegionID idTextureOn, float x, float y,
			Text text, int maxTextVisible, EditAction action) {
		super(idTexture, x, y);
		this.regionOn = RaniaGame.mView.getTextureRegion(idTextureOn);
		this.text = text;
		this.maxTextVisible = maxTextVisible;
	...
	public boolean touchDown(float x, float y) {
		...
		for (int i = beginVisible; i <= endVisible; i++) {
			float posChar = Math.abs(text.getTextBound(beginVisible, i).width
					+ text.position.x + text.getOffset().x - x);
			if (minX > posChar) {
				cursorPos = i;
				minX = posChar;
			}
	...
	public boolean draw(SpriteBatch sprite) {
		...
		if (text != null) {
			updateTextLength();
			text.draw(sprite, beginVisible, endVisible, text.position.x
					+ position.x, text.position.y + position.y, angle, scale.x,
					scale.y);
		}
		...
		if (showCursor) {
			...
			shape.filledRect(
					position.x + text.position.x
							+ text.getTextBound(beginVisible, cursorPos).width
							+ text.getOffset().x - widthCursor * 0.5f,
					position.y + text.position.y - heightCursor * 0.5f,
					widthCursor, heightCursor);
		}
		shape.end();
	...
	public boolean keyTyped(char character) {
		if (readOnly)
			return true;
		changeText(text.content.substring(0, cursorPos) + character
	...
	protected void removeLeftChar() {
		if (cursorPos > 0) {
			changeText(text.content.substring(0, cursorPos - 1)
					+ text.content.substring(cursorPos));
			cursorPos--;
		}
	}
	...
	protected void removeRightChar() {
		if (cursorPos != text.content.length())
			changeText(text.content.substring(0, cursorPos)
					+ text.content.substring(cursorPos + 1));
	}
