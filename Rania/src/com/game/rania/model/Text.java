...
public class Text extends HUDObject {
	...
	public Text(String text, BitmapFont font, Color color, float x, float y,
			Align horzAlign, Align vertAligin) {
		super(x, y, 0, 1, 1);
		this.content = text;
		this.font = font;
		this.color = color;
	...
	public boolean draw(SpriteBatch sprite, float x, float y, float angle,
	...
	public boolean draw(SpriteBatch sprite, int begin, int end, float x,
			float y, float angle, float scaleX, float scaleY) {
		if (!visible || content.isEmpty())
			return false;
	...
	protected void preDraw(float x, float y, float angle, float scaleX,
			float scaleY) {
