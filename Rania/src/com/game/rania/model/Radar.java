...
public class Radar extends HUDObject {
	...
	public boolean touchUp(float x, float y) {
		...
		smallMode = !smallMode;
		if (smallMode) {
			RaniaGame.mController.addProcessor(Controllers.locController
					.getPlayerController());
			...
			RaniaGame.mController.removeProcessor(Controllers.locController
					.getPlayerController());
			player.stop();
	...
	public void update(float deltaTime) {
		...
		textCoord.content = String.format("%d %d",
				locController.getOutputX(player.position.x),
				locController.getOutputY(player.position.y));
		textUsers.content = String.valueOf(locController.getUsers().size());
	}

	private FrameBuffer smallFrameBuffer = null, bigFrameBuffer = null,
			frameBuffer = null;
	private TextureRegion smallRegionBuffer = null, bigRegionBuffer = null,
			regionBuffer = null;
	...
	private void initFrameBuffer() {
		...
		frameBuffer = smallFrameBuffer = new FrameBuffer(Format.RGBA4444,
				region.getRegionWidth(), region.getRegionHeight(), false);
		regionBuffer = smallRegionBuffer = new TextureRegion(
				smallFrameBuffer.getColorBufferTexture());
		smallRegionBuffer.flip(false, true);

		bigFrameBuffer = new FrameBuffer(Format.RGBA4444,
				region.getRegionWidth() * bigCoeff, region.getRegionHeight()
						* bigCoeff, false);
		bigRegionBuffer = new TextureRegion(
				bigFrameBuffer.getColorBufferTexture());
		...
		textPlanet = new Text("", Font.getFont("data/fonts/Postmodern One.ttf",
				20), color, 0, 0);
		textCoord = new Text("", Font.getFont("data/fonts/Postmodern One.ttf",
				20), color, width * 0.5f, 20);
		textUsers = new Text("", Font.getFont("data/fonts/Postmodern One.ttf",
				20), color, width * 0.5f, height - 20);
	}
	...
	public boolean draw(SpriteBatch sprite) {
		...
		if (smallMode) {
			...
			drawRegion(spriteBuffer, region, width * 0.5f, height * 0.5f,
			...
			if (locController.getStar() != null) {
				...
				for (Planet planet : locController.getPlanets()) {
					DrawUtils.drawDottedCircle(shapeBuffer, posObject.x,
							posObject.y, planet.orbit * width / size, 4.0f);
				}
				shapeBuffer.end();
			}
			...
			if (Config.radarNoiseOn && sensorRegion != null) {
				spriteBuffer.setColor(color);
				drawRegion(spriteBuffer, sensorRegion, deltaSensor,
						height * 0.5f, angle, 1,
						height * 0.98f / sensorRegion.getRegionHeight());
			}
			...
			if (objRegion != null) {
				...
				for (Location location : locController.getLocations()) {
					drawRadarObject(location.x, location.y, 0, 0.04f, 0.04f,
							size);
					...
					textPlanet
							.draw(spriteBuffer, posObject.x, posObject.y + 25);
					textPlanet.content = "("
							+ locController.getOutputX(location.x) + ", "
							+ locController.getOutputY(location.y) + ")";
					textPlanet
							.draw(spriteBuffer, posObject.x, posObject.y - 25);
				}
			}
	...
	protected void drawRadarObject(Object object, float fixedSize,
			float gridSize) {
		if (fixedSize == 0) {
			drawRadarObject(
					object.position.x,
			...
			drawRadarObject(object.position.x, object.position.y, object.angle,
					fixedSize, fixedSize, gridSize);
		}
	}

	protected void drawRadarObject(float posX, float posY, float angle,
			float w, float h, float gridSize) {
