...
public class ParallaxObject extends Object {
	...
	public ParallaxObject(RegionID id, float x, float y, float delta) {
		...
		parallaxPosition.set(position.x - camera.position.x * ratio, position.y
				- camera.position.y * ratio);
	}

	public ParallaxObject(RegionID id, float x, float y, float angle,
			float scaleX, float scaleY, float delta) {
		super(id, x, y);
		ratio = delta;
		this.angle = angle;
